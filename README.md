...
* 들여쓰기는 TAB 1개로 합니다.(에디터나 IDE 환경에 영향을 받지만 용량과 효율성 측면에서 TAB을 추천합니다)
...
* 클래스 이름 규칙은 [BEM(Block Element Modifier)](http://getbem.com/naming/)스타일을 추천합니다.
* 클래스 이름은 영문, 숫자, 더블 대시(`--`), 더블 언더스코어(`__`), 대시(`-`)만 사용합니다.
* Block요소와 Element요소 연결시 더블 언더스코어(`__`)를 사용하고, 기존 요소와 Modifier 연결시 더블 대시(`--`)를 사용합니다.
* 단어와 단어 연결시 카멜케이스(camelCase)대신 대시(`-`)를 사용합니다.
